...
export const guest = (() => {
    ...
    const showGuestName = () => {
        ...
        if (name) {
            ...
            const template = `<small class="mt-0 mb-1 mx-0 p-0">${util.escapeHtml(guestName?.getAttribute('data-message'))}</small><p class="m-0 mt-1 p-0" style="font-size: 1.25rem">${util.escapeHtml(name)}</p>`;
    ...
    const buildGoogleCalendar = () => {
        ...
        const data = new URLSearchParams({
            action: 'TEMPLATE',
            text: 'Pernikahan Alvin dan Farah',
            dates: `${formatDate('2025-11-26 09:00')}/${formatDate('2025-11-26 14:00')}`,
            details: 'Tanpa mengurangi rasa hormat, kami mengundang Anda untuk berkenan menghadiri acara pernikahan kami. Terima kasih atas perhatian dan doa restu Anda, yang menjadi kebahagiaan serta kehormatan besar bagi kami.',
            location: 'PP. Al Hikmah Al Hidayah Kedung Cangkring, Jabon, Kabupaten Sidoarjo, Jawa Timur',
            ctz: config.get('tz'),
        });
